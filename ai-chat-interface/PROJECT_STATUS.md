...
### 🚫 더미데이터 사용 금지 정책 (2025-09-20 시행)

**더미데이터(Mock Data) 사용을 전면 금지합니다.**
...
2. **데이터 부족 시**: 실제 없는 데이터는 빈 배열/0으로 표시
3. **개발 단계**: 실제 데이터베이스 스키마와 샘플 데이터를 우선 구축
4. **에러 처리**: 데이터베이스 연결 문제를 명확히 사용자에게 안내
