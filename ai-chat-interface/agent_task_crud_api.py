...
def list_agents(project_id: str):
    ...
    try:
        ...
        result = db.supabase.table('project_agents').select('*').eq('project_id', project_id).eq('framework', framework).eq('is_active', True).order('agent_order').execute()

        return jsonify({'agents': result.data}), 200
...
def create_agent(project_id: str):
    ...
    try:
        ...
        result = db.supabase.table('project_agents').insert(agent_data).execute()

        return jsonify({'agent': result.data[0] if result.data else {}}), 201
...
def update_agent(project_id: str, agent_order: int):
    ...
    try:
        ...
        if 'backstory' in data:
            update_data['backstory'] = data['backstory']
        if 'llmModel' in data:
            update_data['llm_model'] = data['llmModel']
        if 'isVerbose' in data:
            update_data['is_verbose'] = data['isVerbose']
        if 'allowDelegation' in data:
            update_data['allow_delegation'] = data['allowDelegation']
        if 'tools' in data:
            update_data['tools'] = data['tools']
        if 'toolConfig' in data:
            update_data['tool_config'] = data['toolConfig']

        if not update_data:
            return jsonify({'error': 'No fields to update'}), 400

        result = db.supabase.table('project_agents').update(update_data).eq('project_id', project_id).eq('framework', framework).eq('agent_order', agent_order).execute()

        if not result.data:
            return jsonify({'error': 'Agent not found'}), 404

        return jsonify({'agent': result.data[0]}), 200
...
def delete_agent(project_id: str, agent_order: int):
    ...
    try:
        ...
        result = db.supabase.table('project_agents').update({'is_active': False}).eq('project_id', project_id).eq('framework', framework).eq('agent_order', agent_order).execute()
