...
### 📊 **완료된 단계**`;
    ...
    const formatStepResult = (stepNumber, stepResult) => {
        ...
        Object.keys(stepResult).forEach(key => {
            if (!['analysis', 'design', 'code', 'tests', 'summary', 'content'].includes(key)) {
        ...
        try {
            const response = await fetch('/api/metagpt/step', {
                ...
                body: JSON.stringify({
                    ...
                    step: approve ? currentWorkflow.nextStep : currentWorkflow.currentStep,
                    user_response: approve ? 'approve' : 'reject',
