...
from execution_service import handle_crewai_request as handle_crewai_request_service, resume_crewai_execution, resume_metagpt_execution
...
# CrewAI와 MetaGPT 경로도 명시적으로 추가
sys.path.append(os.path.join(project_root, 'CrewAi'))
sys.path.append(os.path.join(project_root, 'MetaGPT'))
...
# Import and register CrewAI blueprint
try:
    from crewai_platform.server import crewai_bp
    app.register_blueprint(crewai_bp)
    print("✅ CrewAI Blueprint 라우트 등록 완료")
except ImportError as e:
    print(f"⚠️ CrewAI Blueprint 라우트 등록 실패: {e}")

# Import and register MetaGPT blueprint
try:
    from metagpt_blueprint import metagpt_bp
    app.register_blueprint(metagpt_bp)
    print("✅ MetaGPT Blueprint 라우트 등록 완료")
except ImportError as e:
    print(f"⚠️ MetaGPT Blueprint 라우트 등록 실패: {e}")
...
PROJECTS_BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'Projects')) # 생성된 프로젝트 저장 경로
...
def handle_crewai_request():
    ...
    data = request.get_json()
    response, status_code = handle_crewai_request_service(data)
...
def get_project_deliverables(project_id):
    """DB에서 프로젝트 산출물(생성된 파일) 목록을 조회합니다."""
    if not supabase:
        return jsonify({'success': False, 'error': 'Database not configured'}), 500
    
    result = supabase.table('project_files').select('*').eq('project_id', project_id).order('file_path').execute()
    
    return jsonify({'success': True, 'deliverables': result.data, 'count': len(result.data)})
