...
def handle_crewai_request():
    ...
    try:
        ...
        script_content = generate_crewai_execution_script(
            requirement=requirement,
            selected_models=selected_models,
...
def generate_crewai_execution_script(requirement: str, selected_models: dict, project_path: str, execution_id: str) -> str:
    """
CrewAI 실행 스크립트 생성 - 단순하고 안전한 방식
    """
    # 요구사항을 Python 문자열로 안전하게 처리
    safe_requirement = repr(requirement[:200] + ('...' if len(requirement) > 200 else ''))
    # repr()로 감싸면 따옴표와 이스케이프가 자동 처리됨
...
print("✅ UTF-8 인코딩 환경 설정 완료")
print(f"🔤 한글 인코딩 테스트: '한글 테스트 문자열'")

print("\ud83d\ude80 CrewAI 실행 시작...")
print("🎯 요구사항:", {safe_requirement})
print(f"\ud83d\udcc1 프로젝트 경로: {project_path}")
print(f"\ud83c\udd94 실행 ID: {execution_id}")
print("\\n" + "="*60 + "\\n")

# LLM 모델 설정
def get_llm_model(role_name: str) -> ChatOpenAI:
    """LLM 모델 반환"""
...
# 에이전트 정의
print("\ud83d\udccb 1. Planner 에이전트 생성...")
planner = Agent(
    role="Project Planner",
    goal="프로젝트 요구사항을 분석하고 체계적인 계획 수립",
    backstory="당신은 프로젝트 관리 전문가로, 복잡한 요구사항을 체계적인 단계로 나누어 정리하는 데 능숙합니다.",
    verbose=True,
    allow_delegation=False,
    llm=get_llm_model("planner")
)

print("\ud83d\udd0d 2. Researcher 에이전트 생성...")
researcher = Agent(
    role="Research Specialist",
    goal="프로젝트에 필요한 기술, 도구, 베스트 프랙티스 조사",
    backstory="당신은 기술 조사 전문가로, 최신 도구와 방법론을 연구하여 프로젝트에 최적의 솔루션을 제안합니다.",
    verbose=True,
    allow_delegation=False,
    llm=get_llm_model("researcher")
)

print("\u270d\ufe0f 3. Writer 에이전트 생성...")
writer = Agent(
    role="Technical Writer",
    goal="연구 결과와 계획을 바탕으로 실제 코드와 문서 작성",
    backstory="당신은 기술 문서 작성 전문가로, 복잡한 기술 내용을 이해하기 쉬운 코드와 문서로 변환할 수 있습니다.",
    verbose=True,
    allow_delegation=False,
...
task1 = Task(
    description=f"""
    다음 요구사항을 분석하고 체계적인 프로젝트 계획을 수립하세요:
    ...
    5. 예상 구현 시간
    """,
    expected_output="체계적이고 세부적인 프로젝트 계획서 (한글로 작성)",
    agent=planner
)

task2 = Task(
    description=f"""
    Planner가 수립한 계획을 바탕으로 다음 사항들을 조사하세요:
    ...
    조사 결과는 실제 구현 가능한 구체적인 방안이어야 합니다.
    """,
...
task3 = Task(
    description=f"""
    Planner의 계획과 Researcher의 조사 결과를 바탕으로 다음을 작성하세요:

    1. 프로젝트 기본 구조 및 파일 구성
    2. 코드 예시 (주요 기능별)
    3. 설치 및 실행 가이드
    4. README.md 파일
    5. requirements.txt 또는 package.json
    6. 기본 테스트 코드

    모든 코드는 실제 작동하는 상태여야 하며, 한글 주석을 포함해야 합니다.
    """,
    expected_output="완성된 프로젝트 코드와 문서 (실행 가능한 형태로 작성)",
    agent=writer
)

task4 = Task(
    description=f"""
    Writer가 작성한 글을 검토하고, 부족한 부분이나 개선점을 구체적으로 제안하세요:

    1. 코드 품질 검토 (구조, 가독성, 효율성)
    2. 문서 완성도 검토 (명확성, 구체성, 실용성)
    3. 누락된 기능이나 요구사항 확인
    4. 개선이 필요한 부분의 구체적 제안
    5. 전체적인 품질 평가 및 추가 개발 가이드

    검토 결과는 Writer가 수정할 수 있도록 구체적이고 실행 가능한 피드백이어야 합니다.
    """,
    expected_output="Writer 작업물에 대한 상세한 검토 보고서 및 개선 제안 (한글로 작성)",
    agent=planner,
    context=[task3]  # Writer의 결과를 참조
)

task5 = Task(
    description=f"""
    Planner의 피드백을 반영하여 글을 다시 수정하고 완성도를 높이세요:

    1. Planner가 제시한 개선점을 모두 반영
    2. 지적된 코드 품질 문제 해결
    3. 문서의 부족한 부분 보완
    4. 누락된 기능이나 요구사항 추가 구현
    5. 전체적인 품질 향상 및 최종 점검

    최종 결과물은 프로덕션 레벨의 완성도를 갖춘 프로젝트여야 합니다.
    """,
    expected_output="Planner 피드백이 반영된 최종 완성 프로젝트 (프로덕션 레벨 품질)",
    agent=writer,
    context=[task3, task4]  # 자신의 이전 작업과 Planner의 피드백을 참조
)
...
try:
    ...
    # 오류 로그 저장 (한글 지원 강화)
    error_file = os.path.join("{project_path}", "error.log")
    with open(error_file, 'w', encoding='utf-8', errors='replace') as f:
        f.write("CrewAI 실행 오류\\n")
        f.write(f"실행 ID: {execution_id}\\n")
        f.write("오류 시간: " + "{{datetime.now()}}\\n")
        f.write("오류 내용: " + "{{str(e)}}\\n")

    sys.exit(1)
'''

    # UTF-8 서로게이트 문제 해결을 위한 안전한 문자열 처리
    import unicodedata

    # 안전한 매개변수 처리
    safe_requirement = requirement.replace('"', '\\"')
    safe_requirement = unicodedata.normalize('NFKC', safe_requirement)
    safe_requirement = ''.join(char for char in safe_requirement if not (0xD800 <= ord(char) <= 0xDFFF))

    safe_project_path = project_path.replace('\\', '\\\\')  # Windows 경로 이스케이프
    safe_project_path = unicodedata.normalize('NFKC', safe_project_path)
    safe_project_path = ''.join(char for char in safe_project_path if not (0xD800 <= ord(char) <= 0xDFFF))

    try:
        formatted_script = script_template.format(
            execution_id=execution_id,
            requirement=safe_requirement,
            safe_requirement=safe_requirement,
            selected_models=selected_models,
        ...
        # 폴백: 기본 스크립트 반환
        return f'''#!/usr/bin/env python3\n# -*- coding: utf-8 -*-\nprint("오류: {str(e)}")\nprint("실행 ID: {execution_id}")'''.encode('utf-8', errors='replace').decode('utf-8')
