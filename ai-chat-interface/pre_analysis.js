(()=> {
  function initPreAnalysis() {
    ...
    async function enableAgentPanel() {
      if (agentPane) {
    ...
    function editAgent(agentOrder) {
      ...
      // Agent의 도구 불러오기
      selectedTools = agent.tools ? [...agent.tools] : [];
    ...
    async function openToolsModal() {
      console.log('[openToolsModal] === CALLED ===');
      const toolsModal = document.getElementById('toolsModal');
      console.log('[openToolsModal] Modal:', toolsModal);
      
      if (!toolsModal) {
        console.error('[openToolsModal] NOT FOUND!');
        return;
      }

      console.log('[openToolsModal] Display before:', toolsModal.style.display);
      
      // Load tools if not loaded yet
      if (!availableMCPs || availableMCPs.length === 0) {
        console.log('[openToolsModal] Loading tools...');
        await loadAvailableTools();
        console.log('[openToolsModal] Loaded:', availableMCPs.length);
      }
      ...
      console.log('[openToolsModal] Display after:', toolsModal.style.display);
      console.log('[openToolsModal] Computed:', window.getComputedStyle(toolsModal).display);
    }
